...
class PrivateAdd(commands.Cog):
    ...
    @app_commands.command(name="private_add", description="Add a user to your private space.")
    @app_commands.describe(channelname="The name of your private space", user="User to add")
    async def add_user(self, interaction: discord.Interaction, channelname: str, user: discord.Member):
        ...
        await interaction.response.send_message(f"✅ {user.mention} added to `{channelname}`!", ephemeral=True)

async def setup(bot: commands.Bot):
    await bot.add_cog(PrivateAdd(bot))
